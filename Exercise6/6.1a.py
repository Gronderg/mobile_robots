...
def compute_control_input(desired_state, robot_state):
    ...
    ang_dist = theta_d - robot_state[2]
    ang_err = ( (ang_dist + np.pi) % (2*np.pi) ) - np.pi
    
    current_input[0] = k*np.sqrt((desired_state[0]-robot_state[0])**2+(desired_state[1]-robot_state[1])**2)
    current_input[1] = k*ang_err
