...
def calculate_gain(desired_state, robot_state):
    mag_err = np.sqrt((desired_state[0] - robot_state[0])**2 + (desired_state[1] - robot_state[1])**2)
    e = math.exp(-beta*mag_err)
    return (v0*(1 - e)/mag_err)                
